...
void ImpedanceController::run(
  ...
  pinocchio::forwardKinematics(pinocchio_model_, pinocchio_data_,
                               robot_configuration, robot_velocity);
  pinocchio::updateFramePlacement(pinocchio_model_, pinocchio_data_,
                                  root_frame_index_);
  pinocchio::updateFramePlacement(pinocchio_model_, pinocchio_data_,
                                  end_frame_index_);
  // Compute the jacobians
  pinocchio::computeJointJacobians(pinocchio_model_, pinocchio_data_,
                                   robot_configuration);
  // Compute relative velocity between frames
  compute_relative_velocity_between_frames(pinocchio_data_, robot_configuration,
                                           robot_velocity);
  run_precomputed_data(pinocchio_data_, gain_proportional, gain_derivative,
                       gain_feed_forward_force, desired_end_frame_placement,
                       desired_end_frame_velocity, feed_forward_force);
}
...
void ImpedanceController::run_precomputed_data(
  ...
  impedance_force_ += (gain_derivative * err_vel_.toVector().array()).matrix();
  impedance_force_ -=
      (gain_feed_forward_force * feed_forward_force.toVector().array())
          .matrix();

  // Get the jacobian.
  pinocchio::getFrameJacobian(pinocchio_model_, pinocchio_data,
                              end_frame_index_, pinocchio::LOCAL_WORLD_ALIGNED,
                              end_jacobian_);
...
void ImpedanceController::run_local(
    Eigen::Ref<const Eigen::VectorXd> robot_configuration,
    Eigen::Ref<const Eigen::VectorXd> robot_velocity,
    Eigen::Ref<const Array6d> gain_proportional,
    Eigen::Ref<const Array6d> gain_derivative,
  ...
  // Actual end frame placement in root frame.
  actual_end_frame_placement_ = pinocchio::SE3(
      Eigen::Matrix3d::Identity(),
      end_placement_.translation() - root_placement_.translation());
  ...
  // Actual end frame velocity in root frame.
  actual_end_frame_velocity_ = pinocchio::Motion(
      relative_vel_between_frames.head(3), Eigen::Vector3d({0, 0, 0}));
...
const Eigen::VectorXd &ImpedanceController::get_torques() { return torques_; }

const Eigen::VectorXd &ImpedanceController::get_joint_torques() {
  return joint_torques_;
}

const ImpedanceController::Vector6d &
...
void ImpedanceController::compute_relative_velocity_between_frames(
    pinocchio::Data &pinocchio_data,
    Eigen::Ref<const Eigen::VectorXd> &robot_configration,
    Eigen::Ref<const Eigen::VectorXd> &robot_velocity) {
  Eigen::Matrix3d root_frame_rotation =
      pinocchio_data_.oMf[root_frame_index_].rotation();
  Eigen::Matrix3d end_frame_rotation =
      pinocchio_data_.oMf[end_frame_index_].rotation();
  pinocchio::SE3 frame_config_root =
      pinocchio::SE3(root_frame_rotation, Eigen::Vector3d::Zero());
  pinocchio::SE3 frame_config_end =
      pinocchio::SE3(end_frame_rotation, Eigen::Vector3d::Zero());
  // root frame jocobian
  root_jacobian_.resize(6, pinocchio_model_.nv);
  root_jacobian_.fill(0.);
  pinocchio::getFrameJacobian(pinocchio_model_, pinocchio_data,
                              root_frame_index_, pinocchio::LOCAL,
                              root_jacobian_);
  pinocchio::getFrameJacobian(pinocchio_model_, pinocchio_data,
                              end_frame_index_, pinocchio::LOCAL,
                              end_jacobian_);
  Eigen::VectorXd vel_root_in_world_frame =
      frame_config_root.toActionMatrix() * root_jacobian_ * robot_velocity;
  Eigen::VectorXd vel_end_in_world_frame =
      frame_config_end.toActionMatrix() * end_jacobian_ * robot_velocity;
  relative_vel_between_frames =
      vel_end_in_world_frame - vel_root_in_world_frame;
}
